...
router.beforeEach(async to => {
  // 开发测试阶段, 可以使用下列语句进行Token的设置
  // 提交代码前一定要将代码进行注释!!!
  // localStorage.set(
  //   STORAGE_TOKEN_KEY,
  //   'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..xkwNXR61kfeSFEiuSR3kGibppOu1z_hGlhQzi9klqiM',
  // );
  ...
  if (!userToken) {
    ...
    if (to.fullPath !== loginRoutePath) {
      // 未登录，进入到登录页
      return {
        path: process.env.VUE_APP_SYCAMORE_WEB_URL + '/user/login?redirect=' + to.fullPath,
        replace: true,
      };
    }
  ...
  if (store.getters['user/allowRouters'] && store.getters['user/allowRouters'].length > 0) {
    ...
    if (!userId) {
      window.location.href =
        process.env.VUE_APP_SYCAMORE_WEB_URL + '/user/login?redirect=' + to.fullPath;
    }
    const info = await store.dispatch(`user/${GET_INFO}`, userId); // 使用当前用户的 权限信息 生成 对应权限的路由表
